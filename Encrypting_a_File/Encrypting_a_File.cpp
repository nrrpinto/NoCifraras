...
int _tmain(int argc, _TCHAR* argv[])
{
    if (argc < 3)
    {
        _tprintf(TEXT("Usage: <example.exe> <source file> <destination file> | <password>\n"));
    ...
    // Give me time to start the monitoring on this process.
    _tprintf(TEXT("Click before encryption starts. \n"));
    _gettch();
...
bool MyEncryptFile(LPTSTR pszSourceFile,LPTSTR pszDestinationFile,LPTSTR pszPassword)
{
    ...
    if (CryptAcquireContext(&hCryptProv, NULL, MS_ENHANCED_PROV, PROV_RSA_FULL, 0))
    {
        if (GetLastError() == NTE_BAD_KEYSET)
        {
            ...
            {
                _tprintf(TEXT("A cryptographic second provider has been acquired. \n"));
            }
            else
            {
    ...
    else
    {
        if (!CryptAcquireContext(&hCryptProv, NULL, NULL, PROV_RSA_FULL, CRYPT_NEWKEYSET))
        {
            _tprintf(TEXT("A cryptographic third provider has been acquired. \n"));
        }
        else
        {
            MyHandleError(TEXT("Error during CryptAcquireContext!\n"), GetLastError());
